...
// Headless frame capture callback — delivers RGBA8 pixel data from any backend.
static std::mutex s_presenter_capture_mutex;
static PresenterFrameCaptureCallback s_presenter_capture_callback;

void SetPresenterFrameCaptureCallback(PresenterFrameCaptureCallback callback)
{
  std::lock_guard<std::mutex> lock(s_presenter_capture_mutex);
  s_presenter_capture_callback = std::move(callback);
}

void ClearPresenterFrameCaptureCallback()
{
  std::lock_guard<std::mutex> lock(s_presenter_capture_mutex);
  s_presenter_capture_callback = nullptr;
}
...
void Presenter::Present(PresentInfo* present_info)
{
  ...
  if (!g_gfx->SupportsUtilityDrawing())
  {
    ...
    // ShowImage() must still be called in headless mode so that registered
    // frame capture callbacks (e.g. OpenEmu integration) receive frame data.
    if (m_xfb_entry)
    {
  ...
  if (g_gfx->IsHeadless())
  {
    // Headless mode: use staging texture readback to deliver frame data via callback.
    // This works with any GPU backend (Metal, Vulkan, etc.).
    if (m_xfb_entry)
    {
      std::lock_guard<std::mutex> lock(s_presenter_capture_mutex);
      if (s_presenter_capture_callback)
      {
        const auto* src_texture = m_xfb_entry->texture.get();
        const u32 width = static_cast<u32>(m_xfb_rect.GetWidth());
        const u32 height = static_cast<u32>(m_xfb_rect.GetHeight());
        const TextureConfig readback_config(width, height, 1, 1, 1,
                                            AbstractTextureFormat::RGBA8, 0,
                                            AbstractTextureType::Texture_2DArray);
        auto staging = g_gfx->CreateStagingTexture(StagingTextureType::Readback, readback_config);
        if (staging)
        {
          staging->CopyFromTexture(src_texture, m_xfb_rect, 0, 0, staging->GetRect());
          staging->Flush();
          if (staging->Map())
          {
            const u32 stride = static_cast<u32>(staging->GetMappedStride());
            s_presenter_capture_callback(
                reinterpret_cast<const u8*>(staging->GetMappedPointer()), width, height, stride);
          }
        }
      }
      SetSuggestedWindowSize(m_xfb_rect.GetWidth(), m_xfb_rect.GetHeight());
