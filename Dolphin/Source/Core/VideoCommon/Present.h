...
// Callback type for headless frame capture: receives (pixel_data, width, height, stride_bytes).
// Pixel data is in RGBA8 format. Called on Dolphin's GPU thread.
using PresenterFrameCaptureCallback = std::function<void(const u8*, u32, u32, u32)>;

// Register / clear a callback to receive frame data when running in headless mode.
// Thread-safe. Works with any GPU backend (Metal, Vulkan, etc.)
