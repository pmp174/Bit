...
// Callback type for frame capture: receives (pixel_data, width, height, stride_bytes)
using FrameCaptureCallback = std::function<void(const u8*, u32, u32, u32)>;

// Set a callback to receive frame data when ShowImage is called.
// Thread-safe. The callback will be invoked on Dolphin's GPU thread.
