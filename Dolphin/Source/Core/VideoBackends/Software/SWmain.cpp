...
{
  // Only attempt to create the OGL window when a render surface is available.
  // In headless mode (e.g. OpenEmu on macOS), GLContext creation succeeds but
  // GLExtensions::Init() crashes calling glGetIntegerv on the AGL context.
  // Frame data can still be captured via the SW::SetFrameCaptureCallback() API.
