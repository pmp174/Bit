...
// Global frame capture callback and its mutex
static std::mutex s_frame_capture_mutex;
static FrameCaptureCallback s_frame_capture_callback;

void SetFrameCaptureCallback(FrameCaptureCallback callback)
{
  std::lock_guard<std::mutex> lock(s_frame_capture_mutex);
  s_frame_capture_callback = std::move(callback);
}

void ClearFrameCaptureCallback()
{
  std::lock_guard<std::mutex> lock(s_frame_capture_mutex);
...
bool SWGfx::BindBackbuffer(const ClearColor& clear_color)
{
  ...
  {
    // Headless mode (e.g. OpenEmu integration): use a fixed backbuffer size
    g_presenter->SetBackbuffer(640, 480);
  }
  return true;
}
...
{
  // Invoke the frame capture callback if one is registered (e.g. OpenEmu integration)
  {
    std::lock_guard<std::mutex> lock(s_frame_capture_mutex);
    if (s_frame_capture_callback)
    {
      const SWTexture* sw_tex = static_cast<const SWTexture*>(source_texture);
      const u32 width = sw_tex->GetConfig().width;
      const u32 height = sw_tex->GetConfig().height;
      const u32 stride = width * 4;  // RGBA8: 4 bytes per pixel
      s_frame_capture_callback(sw_tex->GetData(0, 0), width, height, stride);
    }
  }
...
SurfaceInfo SWGfx::GetSurfaceInfo() const
{
  ...
  {
    GLContext* context = m_window->GetContext();
    return {std::max(context->GetBackBufferWidth(), 1u),
            std::max(context->GetBackBufferHeight(), 1u), 1.0f, AbstractTextureFormat::RGBA8};
  }
  // Headless mode: return fixed surface info
  return {640, 480, 1.0f, AbstractTextureFormat::RGBA8};
}
