...
// Forward declaration for OpenEmu render delegate protocol
...
// Diagnostic logging for OpenEmu bridge
static FILE* s_oeLogFile = nullptr;
static int s_oeFrameCount = 0;
static void OEBridgeLog(const char* fmt, ...) __attribute__((format(printf, 1, 2)));
static void OEBridgeLog(const char* fmt, ...) {
    if (!s_oeLogFile) {
        s_oeLogFile = fopen("/tmp/pcsx2_openemu.log", "a");
        if (!s_oeLogFile) return;
    }
    // Get timestamp
    ...
    fprintf(s_oeLogFile, "[%s] [GS-MTL] ", timebuf);
    va_list args;
    va_start(args, fmt);
    vfprintf(s_oeLogFile, fmt, args);
    va_end(args);
    fprintf(s_oeLogFile, "\n");
    fflush(s_oeLogFile);
}
...
bool GSDeviceMTL::HasSurface()  const { return static_cast<bool>(m_layer) || OpenEmuBridge::g_metalDevice; }

void GSDeviceMTL::AttachSurfaceOnMainThread()
{
	// OpenEmu bridge: skip layer/view setup when using external device
	if (OpenEmuBridge::g_metalDevice)
	{
		Console.WriteLn("Metal: Skipping surface attachment (OpenEmu mode)");
		return;
	}
...
{ @autoreleasepool {
	...
	// OpenEmu bridge: use externally provided Metal device if available
	if (OpenEmuBridge::g_metalDevice)
	{
		Console.WriteLn("Metal: Using OpenEmu-provided Metal device");
		OEBridgeLog("Create: Using OpenEmu Metal device: %s", [[OpenEmuBridge::g_metalDevice name] UTF8String]);
		OEBridgeLog("Create: g_outputTexture: %p (%ux%u)", OpenEmuBridge::g_outputTexture,
			(u32)[OpenEmuBridge::g_outputTexture width], (u32)[OpenEmuBridge::g_outputTexture height]);
		m_dev = GSMTLDevice(MRCRetain(OpenEmuBridge::g_metalDevice));
	}
	else
	{
	...
	{
		// OpenEmu bridge: acquire window info (for dimensions) but skip layer setup
		if (OpenEmuBridge::g_metalDevice)
		{
			// Must call AcquireWindow to set m_window_info dimensions (used by PresentRect)
			if (!AcquireWindow(true))
				return false;
			Console.WriteLn("Metal: OpenEmu mode - window info %ux%u, skipping layer setup",
				m_window_info.surface_width, m_window_info.surface_height);
			OEBridgeLog("Create: OpenEmu mode - window %ux%u, device OK=%d, queue OK=%d",
				m_window_info.surface_width, m_window_info.surface_height,
				(bool)m_dev.dev, (bool)m_queue);
			OEBridgeLog("Create: Shaders loaded: %d", (bool)m_dev.shaders);
			m_vsync_mode = GSVSyncMode::Disabled;
		}
		else
		{
	...
	// In OpenEmu mode, m_layer is nil - use BGRA8Unorm (matching output texture format)
...
{ @autoreleasepool {
	...
	// OpenEmu bridge: render to a temporary texture instead of a drawable
	if (OpenEmuBridge::g_metalDevice && OpenEmuBridge::g_outputTexture)
	{
		...
		{
			if (s_oeFrameCount <= 5)
				OEBridgeLog("BeginPresent[%d]: SKIPPED (frame_skip=%d, g_gs_device=%p)", s_oeFrameCount, frame_skip, g_gs_device.get());
		...
		if (s_oeFrameCount <= 5)
			OEBridgeLog("BeginPresent[%d]: OK - rendering directly to g_outputTexture=%p (%ux%u)",
				s_oeFrameCount, (id)OpenEmuBridge::g_outputTexture,
				(u32)[OpenEmuBridge::g_outputTexture width],
				(u32)[OpenEmuBridge::g_outputTexture height]);
...
{ @autoreleasepool {
	...
	// OpenEmu bridge: readback rendered frame from Shared GPU texture to CPU buffer
	if (OpenEmuBridge::g_metalDevice && OpenEmuBridge::g_outputTexture)
	{
		EndRenderPass();

		// Capture command buffer reference before FlushEncoders nils it
		// MRCOwned copy constructor retains, preventing dangling pointer
		MRCOwned<id<MTLCommandBuffer>> cmdBuf = m_current_render_cmdbuf;
		FlushEncoders();

		// Wait for GPU to finish rendering to the Shared texture
		[cmdBuf waitUntilCompleted];

		if (s_oeFrameCount <= 5)
			OEBridgeLog("EndPresent[%d]: cmdBuf status=%lu error=%s",
				s_oeFrameCount, (unsigned long)[(id<MTLCommandBuffer>)cmdBuf status],
				[[(id<MTLCommandBuffer>)cmdBuf error].localizedDescription UTF8String] ?: "none");

		// Read pixels from Shared texture to CPU video buffer
		u32 w = (u32)[OpenEmuBridge::g_outputTexture width];
		u32 h = (u32)[OpenEmuBridge::g_outputTexture height];
		[OpenEmuBridge::g_outputTexture getBytes:OpenEmuBridge::g_videoBuffer
		                              bytesPerRow:w * 4
		...
		// Signal frame ready for OpenEmu's executeFrame to pick up
		{
			...
			OpenEmuBridge::g_frameReady = true;
		}
		OpenEmuBridge::g_frameCV.notify_one();
