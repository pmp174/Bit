	...
	namespace Core
	{
		...
		void Fds::Adapter::SaveState(State::Saver& state) const
		{
...

			{
				...
				const byte data[16] =
				{
					static_cast<byte>(unit.drive.ctrl),
					static_cast<byte>(unit.drive.status),
					static_cast<byte>(unit.drive.in & 0xFFU),
					static_cast<byte>(unit.drive.out),
			...
			{
				{
					byte data[6] =
					{
						...
						static_cast<byte>(envelopes.length),
						static_cast<byte>(envelopes.counter)
					};
			...
			{
				{
					const byte data[4] =
					{
						...
						static_cast<byte>(modulator.sweep),
						static_cast<byte>(modulator.pos)
					};
